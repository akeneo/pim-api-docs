...
### <span class="label label-news label-added">ADDED</span> Delete category by code

Previously, categories and their subcategories could only be deleted through the user interface. We are now introducing a new endpoint to enable category deletion via the API.

👉 Get more information on the [API reference for this endpoint](https://api.akeneo.com/api-reference.html#delete_categories__code_)
