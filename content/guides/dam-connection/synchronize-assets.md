...
Alright now, let's code! In this section, you'll find everything you need to code your connector.

::: warning
Did you carefully read the previous sections? If not, you're missing some crucial steps and there are chances you won't understand what's coming next. So, here you go: [here](introduction.html), [here](glossary.html), [here](pre-requisites.html) and [here](technical-stack.html).  
We promise it's worth the read. :wink:
:::

As a reminder, the connector is the piece of software that is between the DAM and the PIM and that's in charge of the synchronization of the DAM assets into the PIM, as illustrated in this diagram.
...
Below is the general diagram of what your connector will actually be doing under the hood.

![General workflow](../../img/guides/dam_pim-general-workflow.svg)

It can be summed up into 3 steps:
...
Depending on your DAM capabilities, there are multiple ways to do this (API calls, message queue, ...).
...
To collect those assets, you'll need to first fetch the codes of the PIM asset families that you defined earlier in the [PIM structuration](pre-requisites.html#in-the-pim) step. To do so, you can either:
...
Thanks to the asset family codes, you'll now be able to ask the DAM only for the assets that belong to those families. This is thanks to the preparation you operated on your DAM side by [adding the asset family information on each of your assets](pre-requisites.html#which-asset-family-your-dam-product-assets-belong-to). It will avoid requesting your entire DAM, which would be really useless and counterproductive. :wink:

Don't forget that, when collecting your DAM assets, you should also filter them to only request the **product** assets, and this, thanks to the identification of the product assets you've done earlier [here](pre-requisites.html#which-dam-assets-are-products-related).

## Transform DAM assets into PIM assets

We already saw in a previous [section](/pre-requisites.html#define-the-attributes-of-your-asset-families) that DAM and PIM assets have a totally different structure. 
...
We recommend you use the unique identifier that is used in your DAM system as the code of your PIM asset. In our previous example, this identifier is the `uid`.  
...
::: warning
Don't forget to handle the errors, if the configuration file doesn't match the structure of the DAM or PIM asset.
:::
...
```
To generate an asset attribute value, you are going to need to know 2 things:
...
To answer those questions, fetch each of your attribute by directly calling this [endpoint](/api-reference-asset-manager.html#get_asset_families__code__attributes).
...
1. Fetching the information for the `disclaimer` PIM attribute will tell you that:
    ...
    - This attribute is **localizable**. So you need to know for which PIM locale this information was enriched. Here, in our example, we consider that the DAM does not handle localization and every text properties within the DAM are enriched in English. So we directly write this in the connector: the corresponding locale for localizable attributes will be "en_US".
...
There are some attribute types that will require special steps in order to be handled them properly.
...
Again, don't hesitate to take a look at our [skeleton](https://github.com/akeneo/dam-connector/blob/master/src/Infrastructure/Pim/AttributeOptionsApi.php) to see this in action. :wink:
