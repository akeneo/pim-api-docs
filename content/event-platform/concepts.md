...
In a near future, additional filters and advanced conditions (such as combining multiple filters with `AND` or `OR` operators) will become available
...
```
> If you want to remove the filter (or if you made a mistake), it's always possible to edit your subscription through a call to the "PATCH" method.
