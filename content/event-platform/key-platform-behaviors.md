...
When the platform suspends, revokes, or resumes your subscription, a notification is sent on the channels configured on your subscriber. By default an email is sent to the `technical_email` you provided; you can also opt into a webhook notification on a URL of your choice. If webhook delivery fails and `email` is not in your channels, the platform falls back to email.

See [Notification webhooks](/event-platform/notification-webhooks.html) for the payload format, signature verification, and delivery guarantees.

::: panel-link Configure your notification webhook [Next](/event-platform/notification-webhooks.html)
:::
