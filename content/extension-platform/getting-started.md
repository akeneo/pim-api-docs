...
This generates a `.upsun/config.yaml` in your project root, either from a guided, non-AI walkthrough or by letting the tool analyze your repository and propose a configuration for you. Either way, review the result before deploying — it defines your `routes`, `services` (Postgres, Redis, a queue, as needed), the `applications.app` runtime, and your `build`/`deploy` hooks.
...
or by opening the environment in the Platform Console (`akeneo-extension-platform web`).
