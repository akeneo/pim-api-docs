...
::: warning
Note that the `completenesses` field is available since the 7.0 version, and when the `with_completenesses` query parameter is set to `true`.
:::
...
::: warning
Note that the `quality_scores` field is available since the 7.0 version and when the `with_quality_scores` query parameter is set to `true`.
:::
