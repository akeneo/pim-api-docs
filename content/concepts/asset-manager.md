...
The sections below describe the format of the `linked_data` property for `media_link` and `media_file` Asset attribute type and `auto_tags`.
...
#### Media file attribute
Whenever the attribute type is `media_file`, the `linked_data` field will contain an object.
  ...
  "a_media_file_attribute": [
    {
      ...
      "linked_data": {
        ...
        "mime_type": "image/png",
        "extension": "png",
        "original_filename": "amy_socks_model_picture.png",
        "updated_at": "2026-04-22T07:39:37+0000"
      }
    }
  ]
}
...
Note that the `updated_at` property reflects the last update date of the media file property itself, not the last update date of the asset as a whole.
For example, if only another attribute of the asset is modified, `updated_at` will remain unchanged, even though the asset itself has been updated.

#### Auto tags
Whenever the query parameter `with_asset_auto_tags` is set to `true`, the `linked_data` field will contain an object with the labels of the asset tags generated by auto-tagging.
