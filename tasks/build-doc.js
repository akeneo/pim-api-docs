...
function imageTokenOverride(tokens, idx, options, env, self) {
  return (
    '<img class="img-responsive in-article" alt="' +
    tokens[idx].content +
    '" src="' +
...
md.use(require("markdown-it-container"), "availability", {
  ...
  render: function (tokens, idx) {
    var versionsAndEditions = tokens[idx].info
      .trim()
      .match(/^availability\sversions=(.*)\seditions=(.*)$/);
    var html = "";
    if (tokens[idx].nesting === 1) {
      ...
      html += _.reduce(
        ...
        function (res, version) {
          return (
            res + ' <span class="label label-version">' + version + "</span>"
          );
        },
...
md.use(require("markdown-it-container"), "php-client-availability", {
  ...
  render: function (tokens, idx) {
    var versionsAndEditions = tokens[idx].info
      .trim()
      .match(/^php-client-availability\sversions=(.*)\seditions=(.*)$/);
    var html = "";
    if (tokens[idx].nesting === 1) {
      var versions = versionsAndEditions[1].split(",");
      html += _.reduce(
...
md.use(require("markdown-it-container"), "version-screenshots", {
  ...
  render: function (tokens, idx) {
    var id = tokens[idx].info
      .trim()
      .match(/^version-screenshots\sid="(.*)"\s2\.x.*\s1\.7.*$/);
    var source_v2x = tokens[idx].info
      .trim()
      .match(/^version-screenshots\sid=".*"\s2\.x(.*)\s1\.7.*$/);
    var source_v17 = tokens[idx].info
      .trim()
      .match(/^version-screenshots\sid=".*"\s2\.x.*\s1\.7(.*)$/);
    return tokens[idx].nesting === 1
      ? "<div>" +
          '<ul class="nav nav-tabs nav-tabs-versions" role="tablist">' +
          '<li role="presentation" class="active"><a href="#v2_' +
          id[1] +
          '" aria-controls="v2_' +
          id[1] +
          '" role="tab" data-toggle="tab">Since v2</a></li>' +
          '<li role="presentation"><a href="#v17_' +
          id[1] +
          '" aria-controls="v17_' +
          id[1] +
          '" role="tab" data-toggle="tab">v1.7</a></li>' +
          "</ul>" +
          '<div class="panel panel-default">' +
          '<div class="panel-body">' +
          '<div class="row tab-content">' +
          '<div role="tabpanel" class="col-xs-12 tab-pane active" id="v2_' +
          id[1] +
          '">' +
          md.render(source_v2x[1]) +
          "</div>" +
          '<div role="tabpanel" class="col-xs-12 tab-pane" id="v17_' +
          id[1] +
          '">' +
          md.render(source_v17[1]) +
          "</div>"
      : "</div>\n</div>\n</div>\n</div>\n";
  },
});
md.use(require("markdown-it-container"), "toc", {
  ...
  render: function (tokens, idx) {
    return tokens[idx].nesting === 1
      ? '<div id="navbar" class="col-sm-3 hidden-xs sticky">' +
...
md.use(require("markdown-it-container"), "panel-link", {
  ...
  render: function (tokens, idx) {
    ...
    if (tokens[idx].nesting === 1) {
      // opening tag
      return (
        '<div class="row" style="margin-top: 80px;"><div class="col-sm-offset-3 col-sm-6">' +
        '<div class="panel panel-default panel-btn">' +
        '<a href="' +
        md.utils.escapeHtml(link[1]) +
        '">' +
        '<div class="panel-body">' +
        '<div class="panel-btn-big">' +
        md.utils.escapeHtml(text[1]) +
        "</div>" +
        '<p class="text-center">' +
        md.utils.escapeHtml(linkTitle[1]) +
        "</p>"
      );
    } else {
      // closing tag
...
gulp.task("build-getting-started", ["clean-dist", "less"], function () {
  ...
  return gulp.src("content/getting-started/**/*.md").pipe(
      ...
      return gulp
        ...
        .pipe(
          insert.prepend(
            getTocMarkdown(
              isOnePage,
              pages[path.basename(path.dirname(file.path))].files,
              path.basename(file.path),
              "/getting-started/" + path.basename(path.dirname(file.path))
            ) + "\n"
          )
        )
        ...
        .on("end", function () {
          return gulp
            .src("src/partials/getting-started.handlebars")
            .pipe(
              gulpHandlebars(
                {
                  active_guides: true,
                  ...
                  gettingStartedName:
                    pages[path.basename(path.dirname(file.path))]
                      .gettingStartedName,
                  pimVersion:
                    pages[path.basename(path.dirname(file.path))].pimVersion,
                  availability:
                    pages[path.basename(path.dirname(file.path))].availability,
            ...
            .pipe(
              revReplace({ manifest: gulp.src("./tmp/rev/rev-manifest.json") })
            )
            .pipe(
              gulp.dest(
                "./dist/getting-started/" +
                  path.basename(path.dirname(file.path))
              )
            );
        });
...
gulp.task("build-guides", ["clean-dist", "less"], function () {
  var pages = {
    ...
    "ecommerce-connection": {
      ...
      files: {
        ...
        "step3-reconcile-PIM-data-with-eCommerce-data.md":
          "Reconcile PIM data with eCommerce data",
        "step4-define-your-first-scope.md":
          "Define the first scope of your connector",
      },
    },
    ...
    "syndication-connection": {
      title:
        "The complete guide to connect Akeneo PIM to your syndication solution",
      files: {
        "introduction.md": "Introduction",
        "step0-who-is-your-connector-for.md": "Who is your connector for?",
        "step1-who-does-what.md": "Who does what?",
    ...
    "print-connection": {
      ...
      files: {
        ...
        "step1-who-does-what.md": "Who does what?",
        "step2-understand-akeneo-pim.md": "Understand Akeneo PIM data",
        "step3-reconcile-PIM-data-with-print-features.md":
          "Reconcile PIM data with print capabilities",
  ...
  return gulp.src("content/guides/**/*.md").pipe(
    flatmap(function (stream, file) {
      return gulp
        ...
        .pipe(
          insert.prepend(
            getTocMarkdown(
              ...
              pages[path.basename(path.dirname(file.path))].files,
              path.basename(file.path),
              "/guides/" + path.basename(path.dirname(file.path))
            ) + "\n"
          )
        )
        .pipe(gulpMarkdownIt(md))
        ...
        .on("end", function () {
          return gulp
            .src("src/partials/documentation.handlebars")
            .pipe(
              gulpHandlebars(
                {
                  active_guides: true,
            ...
            .pipe(
              gulp.dest(
                "./dist/guides/" + path.basename(path.dirname(file.path))
              )
            );
        });
    })
...
gulp.task("build-rest-api", ["clean-dist", "less"], function () {
  ...
  return gulp.src("content/rest-api/*.md").pipe(
    flatmap(function (stream, file) {
      return gulp
        .src("content/rest-api/*.md")
        .pipe(insert.wrap("::::: mainContent\n", "\n:::::"))
        .pipe(
          insert.prepend(
            getTocMarkdown(
...
gulp.task("build-events-api", ["clean-dist", "less"], function () {
  ...
  return gulp.src("content/events-api/*.md").pipe(
    flatmap(function (stream, file) {
      return gulp
        .src("content/events-api/*.md")
        .pipe(insert.wrap("::::: mainContent\n", "\n:::::"))
        .pipe(
          insert.prepend(
            getTocMarkdown(
        ...
        .on("end", function () {
          return gulp
            .src("src/partials/events-documentation.handlebars")
            .pipe(
              gulpHandlebars(
                {
                  active_documentation: true,
...
gulp.task("build-apps", ["clean-dist", "less"], function () {
  var pages = {
    ...
    "how-to-have-public-url-for-my-app.md":
      "How to have a public URL for my App?",
    "create-app-in-php.md": "Create an App in PHP",
    "create-app.md": "Create an App in any language",
  };

  var isOnePage = false;

  return gulp.src("content/apps/*.md").pipe(
    flatmap(function (stream, file) {
      return gulp
        .src("content/apps/*.md")
        .pipe(insert.wrap("::::: mainContent\n", "\n:::::"))
        ...
        .on("end", function () {
          return gulp
            .src("src/partials/apps.handlebars")
            .pipe(
              gulpHandlebars(
                {
                  active_documentation: true,
...
gulp.task("build-concepts", ["clean-dist", "less"], function () {
  ...
  return gulp.src("content/concepts/*.md").pipe(
    flatmap(function (stream, file) {
      return gulp
...
gulp.task("create-catalog-structure-md", function () {
  return gulp
    .src(["content/php-client/resources/catalog-structure/*.md"])
...
gulp.task("create-target-market-settings-md", function () {
  return gulp
    .src(["content/php-client/resources/target-market-settings/*.md"])
...
gulp.task("create-PAM-md", function () {
  return gulp
    .src(["content/php-client/resources/PAM/*.md"])
    .pipe(concat("PAM.md"))
...
gulp.task("create-reference-entity-md", function () {
  return gulp
    .src([
      ...
      "content/php-client/resources/reference-entity/*.md",
    ])
...
gulp.task("create-asset-manager-md", function () {
  return gulp
    .src([
      ...
      "content/php-client/resources/asset-manager/*.md",
    ])
...
gulp.task(
  ...
  [
    "create-products-md",
    "create-catalog-structure-md",
    "create-target-market-settings-md",
    "create-PAM-md",
    "create-reference-entity-md",
    "create-asset-manager-md",
  ],
  function () {
    return gulp
      .src([
        "tmp/php-client-resources/products.md",
...
gulp.task(
  "build-php-client",
  ["clean-dist", "less", "create-resources-md"],
  function () {
    var pages = {
    ...
    return gulp
      .src(["content/php-client/*.md", "tmp/php-client/resources.md"])
      .pipe(
        flatmap(function (stream, file) {
          return gulp
            .src(file.path)
            ...
            .on("end", function () {
              return gulp
                ...
                .pipe(
                  revReplace({
                    manifest: gulp.src("./tmp/rev/rev-manifest.json"),
                  })
                )
...
gulp.task("build-misc-documentation", ["clean-dist", "less"], function () {
  ...
  return gulp.src(["content/misc/*.md"]).pipe(
    flatmap(function (stream, file) {
      return gulp
        .src(file.path)
        .pipe(insert.wrap("::::: mainContent\n", "\n:::::"))
        .pipe(
          insert.prepend(
            getTocMarkdown(isOnePage, path.basename(file.path)) + "\n"
          )
        )
        .pipe(gulpMarkdownIt(md))
        ...
        .on("end", function () {
          return gulp
            .src("src/partials/misc.handlebars")
            .pipe(
              gulpHandlebars(
                {
                  active_documentation: true,
